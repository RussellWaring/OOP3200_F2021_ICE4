 ...
 * @date	2021-10-07
 * @desc	The entry point for testing these gameObjects baby!!
 */
...
// #include "Vector2D.h" - don't need to include as it is included in GameObject

static void BuildGameObjects(std::vector<GameObject*>& game_objects, const int num = 2) // New function, pass a standard vector of type game objects
{
...
static void CompareGameObjects(GameObject* object1, GameObject* object2)
{
	...
	std::cout << "Magnitude of first gameObject is: " << object1->GetPosition().GetMagnitude() << std::endl; // accessing gameObject inside of vector. -> get game object that's sitting inside here. Arrow instead of dot (pointer)
...
int main()
{
	std::vector<GameObject*> gameObjects; // a vector to pointer-gameObjects
	...
	// Call function to query gameObjects from vector
...
/* Looking forward:
 * There are no checks
 * Can't enter it all at once
 * Compare by id, not by index
 * FindObjectById() method
 * Construction class? -> Factory class?
 */
